/**
 * ============================================================================
 * INFORME TÉCNICO DE INVESTIGACIÓN Y DESARROLLO (DOCUMENTO EXTENSO APA 7ma)
...
drawH1('1. INTRODUCCIÓN Y JUSTIFICACIÓN');
drawParagraph(
  'El vertiginoso avance en el desarrollo de aplicaciones móviles ha transformado la arquitectura del software contemporáneo, transitando desde soluciones monolíticas locales hacia sistemas distribuidos basados en microservicios y clientes móviles enriquecidos. En este ecosistema, los ingenieros y desarrolladores de software se enfrentan a desafíos críticos tales como la fragmentación de dispositivos, la heterogeneidad de sistemas operativos, la inestabilidad inherente de las redes móviles celulares y las exigencias de seguridad criptográfica para la distribución de paquetes ejecutables.'
);
drawParagraph(
  'El presente trabajo de investigación analiza y resuelve de manera teórica y práctica los componentes esenciales del ciclo de desarrollo móvil (Mobile Development Life Cycle - MDLC): en primer lugar, se evalúan los entornos de pruebas comparando dispositivos reales, emuladores basados en hipervisores (QEMU/AVD) y simuladores de software; en segundo lugar, se profundiza en la comunicación cliente-servidor mediante APIs RESTful utilizando el cliente HTTP Axios y su mecanismo de interceptores; y en tercer lugar, se detalla el pipeline de compilación, optimización con zipalign y firmado digital con apksigner para la generación de paquetes APK en Android.'
);
...
drawParagraph(
  'Analizar, contrastar y aplicar los fundamentos teóricos y prácticos de los entornos de ejecución móvil (reales y virtuales), el consumo de servicios web REST mediante la biblioteca Axios y el proceso de compilación, empaquetado y firmado criptográfico de archivos APK para Android, a través de una investigación exhaustiva y el desarrollo de una solución práctica modular según los estándares de ingeniería de software.'
);

drawH2('2.2. Objetivos Específicos');
drawBullet('Investigar los principios de funcionamiento, arquitectura computacional de bajo nivel, ventajas y desventajas de dispositivos físicos reales, emuladores basados en hipervisores (QEMU/AVD) y simuladores de software.');
drawBullet('Examinar la arquitectura RESTful y el protocolo HTTP/HTTPS en entornos móviles, evaluando la biblioteca Axios frente a Fetch nativo en aspectos de transformación automática de JSON, interceptores globales, gestión de timeouts y control de errores.');
drawBullet('Detallar el pipeline de compilación de Android (bytecode DEX, compilador D8/R8, empaquetador AAPT2, Gradle) y el procedimiento de firmado digital con keytool, zipalign y apksigner bajo esquemas v1, v2 y v3.');
drawBullet('Implementar una aplicación móvil modular y funcional que consuma una API REST pública en tiempo real mediante Axios, integrando pruebas automatizadas y scripts de construcción y despliegue en entornos reales y virtuales.');
...
drawParagraph(
  'La etapa de pruebas y aseguramiento de la calidad (QA) en el software móvil requiere validar el comportamiento de las aplicaciones ante una gran diversidad de factores: distintas resoluciones de pantalla, densidades de píxeles (DPI), arquitecturas de CPU (ARMv7, ARM64-v8a, x86_64), capacidades de memoria RAM y versiones del sistema operativo. Para abordar esta complejidad, la industria recurre a tres entornos de ejecución: dispositivos reales, emuladores y simuladores.'
);

drawH3('3.1.1. Dispositivo Móvil Real (Hardware Físico Nativo)');
drawParagraph(
  '¿Qué es? Un dispositivo móvil real es una unidad física comercial de smartphone, tablet o dispositivo embebido que integra un System-on-Chip (SoC) basado en arquitectura ARM o ARM64, memoria RAM volátil, almacenamiento flash NAND, circuitos de radiofrecuencia (módem celular 4G/5G, Wi-Fi 6, Bluetooth Low Energy, NFC) y un conjunto de sensores físicos (GPS asistido por satélites GLONASS/Galileo, acelerómetro triaxial, giroscopio, magnetómetro, sensor barométrico, sensores de iluminación ambiental, cámaras de alta resolución y lectores biométricos de huella o faciales).'
);
drawParagraph(
  '¿Cómo funciona y su Arquitectura Técnica? La aplicación se instala y ejecuta de forma directa sobre la pila de software nativa del fabricante. El código empaquetado en bytecode DEX es interpretado y compilado por el Android Runtime (ART) mediante compilación híbrida AOT (Ahead-of-Time) y JIT (Just-in-Time) con soporte de perfiles de optimización en tiempo de ejecución. ART interactúa directamente con el kernel de Linux y los controladores de la Capa de Abstracción de Hardware (HAL). La comunicación de desarrollo y depuración se gestiona a través del protocolo Android Debug Bridge (ADB), el cual establece una conexión cliente-servidor mediante interfaces físicas USB 3.0 o canales TCP/IP inalámbricos (ADB over Wi-Fi).'
);
drawParagraph(
...
drawParagraph(
  'En la ingeniería de virtualización existe una distinción técnica profunda entre los conceptos de emulación y simulación:'
);
drawBullet('Emulador (Virtualización de Hardware y Software): Un emulador reproduce con exactitud matemática tanto el comportamiento del hardware subyacente como la pila de software del sistema objetivo. Modela el conjunto de instrucciones del procesador (ISA), los registros de la CPU, la gestión de memoria MMU, los controladores de bus e interrupciones mediante hipervisores y traducción binaria (Binary Translation). El Android Virtual Device (AVD) es un emulador que ejecuta el kernel Linux real de Android y todo el sistema operativo sobre QEMU 2.0+ con aceleradores de hipervisor tipo 2 como KVM en sistemas Linux o Hyper-V/HAXM en Windows.');
drawBullet('Simulador (Simulación de Software y APIs de Alto Nivel): Un simulador no intenta recrear el hardware subyacente. En su lugar, recrea únicamente las interfaces de programación de aplicaciones (APIs), las bibliotecas de tiempo de ejecución y el comportamiento de la interfaz gráfica sobre el hardware y el kernel del equipo anfitrión (Host). Un ejemplo paradigmático es el iOS Simulator en Xcode, el cual ejecuta código compilado para la arquitectura x86_64 o ARM de la computadora Mac y simula las llamadas a la biblioteca Cocoa Touch, pero no ejecuta el kernel móvil real de iOS.');
...
drawBullet('resources.arsc: Tabla binaria de recursos precompilados que contiene cadenas de texto, identificadores numéricos, temas y configuraciones de diseño.');
